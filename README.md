...
- ArcGIS FeatureServer `/query` URLs are accepted and normalized to their layer endpoint; `where` and `outFields` parameters are applied to the native feature layer. The SDK projects service geometry into the map's spatial reference automatically.
